...
To train a neural network you really need only a few things:
...
{% include info.html text="$\textbf{Notation}$: I'm using `dfunc` in code to indicate that the variable stores $\frac{\partial \text{MSE}}{\partial \text{func}}$. For example,`drelu` in this case is the derivative of the loss in respect to the outputs of the ReLU function: $\text{drelu} = \frac{\partial \text{MSE}}{\partial \text{ReLU}}$." %}
...
For backward pass we only need to save gradients for parameters that we want to update.
...
There are several ways to verify the model's behaviour, but I think the best method here is to just look at the data. We are going to generate data from one dimensional data, split it into training and validation sets, fit the model to the training set, and plot it all to see the results.
...
I would recommend playing[^3] around with different dataset sizes, learning rates and number of neurons in a hidden layer to see where the training starts to break. Doing that should allow one to get a feeling of what needs to be done to improve it all and understand why modern techniques were developed.
